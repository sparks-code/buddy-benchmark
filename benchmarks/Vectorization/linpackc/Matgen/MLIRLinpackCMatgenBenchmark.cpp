...
static void Matgen_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      matgen(MLIRMatgen_af32.getData(), lda, n_matgen, MLIRMatgen_bf32.getData(), MLIRMatgen_normaf32.getData());
    }
  }
}

static void Matgen_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      matgen(MLIRMatgen_af64.getData(), lda, n_matgen, MLIRMatgen_bf64.getData(), MLIRMatgen_normaf64.getData());
    }
  }
}
...
BENCHMARK(Matgen_F32)->Arg(1);
BENCHMARK(Matgen_F64)->Arg(1);
...
// bool testPadMessage(std::size_t Len) {
//     unsigned char* str = randomString(Len);
//     unsigned char* padded_message = pad_message(str, Len);
//     std::size_t padded_len = padded_length(Len);

//     MemRef<unsigned char, 1> input{str, static_cast<intptr_t>(Len)};
//     MemRef<unsigned char, 1> output{padded_len};
//     _mlir_ciface_mlir_padmessage(&output, &input, Len);

//     for(std::size_t i = 0; i < padded_len; ++i){
//         assert(output[i] == padded_message[i]);
//     }
//     delete[] str;
