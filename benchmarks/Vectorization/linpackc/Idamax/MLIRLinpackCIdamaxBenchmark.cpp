...
static void idamax_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      idamax(n, inputMLIRidamax_dxf32.getData(), input_incx);
    }
  }
}
static void idamax_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
       idamax(n, inputMLIRidamax_dxf64.getData(), input_incx);
    }
  }
}
...
BENCHMARK(idamax_F32)->Arg(1);
BENCHMARK(idamax_F64)->Arg(1);
