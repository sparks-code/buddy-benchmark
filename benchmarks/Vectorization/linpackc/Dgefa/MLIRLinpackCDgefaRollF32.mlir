...
func.func @get_offset(%lda : index, %i : index, %j : index) -> index {
  // m[lda*i+j];
  %lda_mi = arith.muli %lda, %i : index
  %lda_mi_aj = arith.addi %lda_mi, %j : index
...
//   if (nm1 >=  0) {
  ...
  // 			ipvt[k] = l;
      ...
      %offset_0 = func.call @get_val(%lda_index, %k_0_index, %k_0_index ) 
                    : (index, index, index) -> index
      %a_offset_0 = memref.subview %a[%offset_0][] : memref<?xf32> to memref<?xf32>
      %l_temp = func.call @mlir_linpackcidamaxf32(%n_sk, %a_offset_0, %i1) //地址怎么传递
      ...
      scf.if %cond2 {
          ...
          scf.if %cond3{
            ...
            func.call @set_val(%a, %lda_index, %k_0_index, %l_index, %a_val_0) 
                : (memref<?xf32>, index, index, index, f32) -> ()
            func.call @set_val(%a, %lda_index, %k_0_index, %k_0_index, %a_val_1 ) 
                : (memref<?xf32>, index, index, index, f32) -> ()
          }
          ...
          %a_val_3 = func.call @get_val(%a, %lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (memref<?xf32>, index, index, index) -> f32
          %offset_3 = func.call @get_val(%lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
          %a_offset_3 = memref.view %a[%offset_3][] : memref<?xf32> to memref<?xf32>
          //   t = -ONE/a[lda*k+k];
          ...
          scf.for %j_0_index = %k_0_index_p1 to %n_index step %c1{
            ...
            // daxpy(n-(k+1),t,&a[lda*k+k+1],1,
					  //     &a[lda*j+k+1],1);     
            %offset_4 = func.call @get_val(%lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
            %a_val_4 = memref.view %a[%offset_4][] : memref<?xf32> to memref<?xf32>
            %offset_5 = func.call @get_val(%lda_index, %j_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
            %a_val_5 = memref.view %a[%offset_5][] : memref<?xf32> to memref<?xf32>
            func.call @daxpy(%n_sk_p1, %t_2, %a_val_4, %i1, %a_val_5, %i1) 
                    : ( i32, f32,  memref<?xf32>,  i32, memref<?xf32>, i32) -> ()
          }
      }else{
           //*info = k;
            memref.store %k_0, %info[%c0] : memref<i32>
      }
    }
  }
  // ipvt[n-1] = n-1;
    ...
    scf.if %cond4 {
       memref.store %n_s1, %info[%c0] : memref<i32>
    }
