...
//   if (nm1 >=  0) {
  ...
  // 			ipvt[k] = l;
      ...
      %offset_0 = func.call @get_offset(%lda_index, %k_0_index, %k_0_index ) 
      ...
      scf.if %cond2 {
          ...
          %a_val_3 = func.call @get_val(%a, %lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (memref<?xf32>, index, index, index) -> f32
          %offset_3 = func.call @get_offset(%lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
          ...
          scf.for %j_0_index = %k_0_index_p1 to %n_index step %c1{
            // t = a[lda*j+l]; 
            %t_2 = func.call @get_val(%a, %lda_index, %j_0_index, %l_index ) 
                    : (memref<?xf32>, index, index, index) -> f32
            //   if (l != k) {
            // 	a[lda*j+l] = a[lda*j+k];
            // 	a[lda*j+k] = t;
            ...
            // daxpy(n-(k+1),t,&a[lda*k+k+1],1,
					  //     &a[lda*j+k+1],1);     
            %offset_4 = func.call @get_offset(%lda_index, %k_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
            ...
            %offset_5 = func.call @get_offset(%lda_index, %j_0_index, %k_0_index_p1 ) 
                    : (index, index, index) -> index
