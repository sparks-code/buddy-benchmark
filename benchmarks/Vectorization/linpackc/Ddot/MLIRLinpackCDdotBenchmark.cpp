...
static void Ddot_ROLL_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      ddot_ROLL(n,inputMLIRDdot_dxf32.getData(),input_incx,inputMLIRDdot_dyf32.getData(),input_incy);
    }
  }
}

static void Ddot_ROLL_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      ddot_ROLL(n,inputMLIRDdot_dxf64.getData(),input_incx,inputMLIRDdot_dyf64.getData(),input_incy);
    }
  }
}

static void Ddot_UNROLL_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      ddot_UNROLL(n,inputMLIRDdot_dxf32.getData(),input_incx,inputMLIRDdot_dyf32.getData(),input_incy);
    }
  }
}

static void Ddot_UNROLL_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      ddot_UNROLL(n,inputMLIRDdot_dxf64.getData(),input_incx,inputMLIRDdot_dyf64.getData(),input_incy);
    }
  }
}
...
BENCHMARK(Ddot_ROLL_F32)->Arg(1);
BENCHMARK(Ddot_ROLL_F64)->Arg(1);
BENCHMARK(Ddot_UNROLL_F32)->Arg(1);
BENCHMARK(Ddot_UNROLL_F64)->Arg(1);
