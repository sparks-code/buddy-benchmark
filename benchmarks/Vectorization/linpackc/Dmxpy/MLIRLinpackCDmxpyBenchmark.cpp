...
MemRef<float, 1> inputMLIRDmxpy_xf64(sizesArrayMLIRLinpackCDmxpy, 1.0);
MemRef<float, 1> inputMLIRDmxpy_yf64(sizesArrayMLIRLinpackCDmxpy, 2.0);
MemRef<float, 1> inputMLIRDmxpy_mf64(sizesArrayMLIRLinpackCDmxpy, 3.0);
...
static void MLIR_DmxpyF64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      _mlir_ciface_mlir_linpackcdmxpyf32(n1, &inputMLIRDmxpy_yf64,
                                        n2, ldm,&inputMLIRDmxpy_xf64, &inputMLIRDmxpy_mf64);
    }
  }
}
static void Dmxpy_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      dmxpy(n1, inputMLIRDmxpy_yf32.getData(), n2, ldm,inputMLIRDmxpy_xf32.getData(), inputMLIRDmxpy_mf32.getData());
    }
  }
}
static void Dmxpy_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      dmxpy(n1, inputMLIRDmxpy_yf64.getData(), n2, ldm,inputMLIRDmxpy_xf64.getData(), inputMLIRDmxpy_mf64.getData());
    }
  }
}
...
BENCHMARK(Dmxpy_F32)->Arg(1);
BENCHMARK(Dmxpy_F64)->Arg(1);
