...
static void Dscal_Roll_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      dscal_ROLL(n, input_dscal_da_f32,inputMLIRDscal_dxf32.getData(), input_incx);
    }
  }
}

static void Dscal_Unroll_F32(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
     dscal_UNROLL(n, input_dscal_da_f32,inputMLIRDscal_dxf32.getData(), input_incx);
    }
  }
}
static void Dscal_Roll_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      dscal_ROLL(n, input_dscal_da_f64,inputMLIRDscal_dxf64.getData(), input_incx);
    }
  }
}
static void Dscal_Unroll_F64(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
     dscal_UNROLL(n, input_dscal_da_f64,inputMLIRDscal_dxf64.getData(), input_incx);
    }
  }
}
...
BENCHMARK(Dscal_Roll_F32)->Arg(1);
BENCHMARK(Dscal_Unroll_F32)->Arg(1);
BENCHMARK(Dscal_Roll_F64)->Arg(1);
BENCHMARK(Dscal_Unroll_F64)->Arg(1);
